...
export default function CompanyMessagesPage() {
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0 h-screen">
      <Header
        title="Messages"
        subtitle="Direct collaboration chat with creators regarding briefs and post drafts."
      />
              ...
              conversations.map((conv) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-4 transition-colors cursor-pointer ${isSelected ? 'bg-white border-l-4 border-blue-600 shadow-xs' : 'hover:bg-gray-100'
                      }`}
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-blue-600 to-indigo-600 text-white font-bold flex items-center justify-center text-xs shrink-0">
                        {creatorName.slice(0, 2).toUpperCase()}
                      </div>
                      <div className="min-w-0 flex-1">
                        <div className="text-xs font-bold text-[#111827] truncate">{creatorName}</div>
                        <div className="text-[11px] text-blue-600 truncate">{campaignTitle}</div>
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-white">
          {activeConversation ? (
            <>
              ...
              <div className="p-4 border-b border-[#E5E7EB] flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-[#6B7280]">
                    Campaign: {activeConversation.collaboration?.campaign?.title} (€{activeConversation.collaboration?.fixedRate})
                  </p>
                </div>
              </div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-end gap-2 max-w-lg">
                        {!isMe && (
                          <div className="w-7 h-7 rounded-full bg-gray-200 flex items-center justify-center text-[10px] font-bold shrink-0">
                            {msg.sender.name.slice(0, 2).toUpperCase()}
                          </div>
                        )}
                        <div
                          className={`p-3.5 rounded-2xl text-xs leading-relaxed shadow-2xs ${isMe
              ...
              <form onSubmit={handleSendMessage} className="p-4 border-t border-[#E5E7EB] bg-white flex items-center gap-3">
                <input
                  ...
                  className="flex-1 bg-[#F9FAFB] border border-[#E5E7EB] rounded-xl px-4 py-2.5 text-xs text-[#111827] focus:bg-white focus:outline-none focus:border-blue-600"
                />
                <button
                  type="submit"
                  disabled={sending || !newMessage.trim()}
                  className="bg-blue-600 hover:bg-blue-700 text-white p-2.5 rounded-xl shadow-xs transition-all disabled:opacity-50 cursor-pointer"
                >
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center p-8 text-[#9CA3AF]">
              ...
              <p className="text-xs">Select a collaboration on the left to view the conversation</p>
            </div>
          )}
        </div>
      </div>
